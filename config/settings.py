...
# Streamlit secrets are only valid on Streamlit Cloud
...
def get_env_var(name: str, default: str = None) -> str:
    return os.getenv(name) or ST_SECRETS.get(name, default)

class settings:
    OPENAI_API_KEY = get_env_var("OPENAI_API_KEY")
    NEO4J_URI = get_env_var("NEO4J_URI")
    NEO4J_USERNAME = get_env_var("NEO4J_USERNAME")
    NEO4J_PASSWORD = get_env_var("NEO4J_PASSWORD")
    TMDB_API_KEY = get_env_var("TMDB_API_KEY")
    OMDB_API = get_env_var("OMDB_API")
    OPENAI_ENDPOINT = get_env_var("OPENAI_ENDPOINT", "https://api.openai.com/v1")
